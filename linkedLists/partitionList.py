...
class LinkedList:
    ...
    def partition_list(self,x):
        ...
        dummyBigHead = None
        while nextNode != None:
            if nextNode.value < x and dummySmall == None:
                dummySmall = nextNode
                self.head = dummySmall
            elif nextNode.value < x and dummySmall != None:
                dummySmall.next = nextNode
                dummySmall = nextNode
            elif nextNode.value >= x and dummyBig == None:
                dummyBig = nextNode
                dummyBigHead = dummyBig
            elif nextNode.value >= x and dummyBig != None:
                dummyBig.next = nextNode
                dummyBig = nextNode
            else:
                print("error! This shouldn't be activated")
            if nextNode.next !=None:
                print(nextNode.value,nextNode.next.value)
            else:
                print(nextNode.value,nextNode.next)
            
            nextNode = nextNode.next
            
                
        if dummySmall != None and dummyBigHead != None:
            dummyBig.next = None
            dummySmall.next = dummyBigHead
...
def test_partition_list():
    ...
    ll.append(2)
    ll.append(1)
    print("Before:", linkedlist_to_list(ll.head),"Getting stuck below")
    ll.partition_list(x)
    print("After:", linkedlist_to_list(ll.head))
